...
if __name__ == '__main__':
    ...
    is_ok(dom.at('p').child_nodes.last().preceding_siblings().first().content, 'test', 'right content')
    is_ok(dom.at('p').child_nodes.last().preceding_siblings().last().content, '123', 'right content')
    is_ok(dom.at('p').child_nodes.last().preceding_siblings().size, 2, 'right number of nodes')
    ...
    is_ok(dom.child_nodes.first().following_siblings().first().type, 'p', 'right type')
    is_ok(dom.child_nodes.first().following_siblings().last().content, 'after', 'right content')
    is_ok(dom.child_nodes.first().following_siblings().size, 2, 'right number of nodes')
    ...
    is_ok(dom.at('p').child_nodes.last().previous_sibling.previous_sibling.content, 'test', 'right content')
    is_ok(dom.at('p').child_nodes.first().next_sibling.next_sibling.content, ' 456 ', 'right content')
    is_ok(dom.descendant_nodes[0].node, 'doctype', 'right node')
    is_ok(dom.descendant_nodes[0].content, ' before', 'right content')
    is_ok(dom.descendant_nodes[0], '<!DOCTYPE before>', 'right content')
    is_ok(dom.descendant_nodes[1].type, 'p', 'right type')
    is_ok(dom.descendant_nodes[2].node, 'text', 'right node')
    is_ok(dom.descendant_nodes[2].content, 'test', 'right content')
    is_ok(dom.descendant_nodes[5].node, 'pi', 'right node')
    is_ok(dom.descendant_nodes[5].content, 'after', 'right content')
    is_ok(dom.at('p').descendant_nodes[0].node, 'text', 'right node')
    is_ok(dom.at('p').descendant_nodes[0].content, 'test', 'right node')
    is_ok(dom.at('p').descendant_nodes.last().node, 'comment', 'right node')
    is_ok(dom.at('p').descendant_nodes.last().content, ' 456 ', 'right node')
    is_ok(dom.child_nodes[1].child_nodes.first().parent.type, 'p', 'right type')
    ...
    is_ok(dom.at('p').child_nodes.first().node, 'text', 'right node')
    is_ok(dom.at('p').child_nodes.first().remove().type, 'p', 'right type')
    is_ok(dom.at('p').child_nodes.first().node, 'cdata', 'right node')
    is_ok(dom.at('p').child_nodes.first().content, '123', 'right content')
    is_ok(dom.at('p').child_nodes[1].node, 'comment', 'right node')
    is_ok(dom.at('p').child_nodes[1].content, ' 456 ', 'right content')
    is_ok(dom.child_nodes[0].node, 'doctype', 'right node')
    is_ok(dom.child_nodes[0].content, ' before', 'right content')
    is_ok(dom.child_nodes[2].node, 'pi', 'right node')
    ...
    is_ok(dom.child_nodes.grep(lambda i: i.node == 'pi').map('remove').first().node, 'root', 'right node')
    ...
    is_ok(dom.at('b').child_nodes.first().ancestors().map('type').join(','), 'b,i,script', 'right result')
    ...
    ok(not dom.at('title').match('book > nons > section'), 'element did not match')
    ...
    is_ok(dom.at('[test]').type, 'div', 'right type')
    is_ok(dom.at('[test2]').type, 'div', 'right type')
    ...
    """)
    is_ok(dom.find(':root')[0].type, 'form', 'right type')
    is_ok(dom.find('*:root')[0].type, 'form', 'right type')
    is_ok(dom.find('form:root')[0].type, 'form', 'right type')
    ...
    is_ok(dom.find(':nth-child(odd)')[0].type, 'ul', 'right type')
    is_ok(dom.find(':nth-child(odd)')[1].text, 'A', 'right text')
    is_ok(dom.find(':nth-child(1)')[0].type, 'ul', 'right type')
    is_ok(dom.find(':nth-child(1)')[1].text, 'A', 'right text')
    is_ok(dom.find(':nth-last-child(odd)')[0].type, 'ul', 'right type')
    is_ok(dom.find(':nth-last-child(odd)').last().text, 'H', 'right text')
    is_ok(dom.find(':nth-last-child(1)')[0].type, 'ul', 'right type')
    ...
    is_ok(element.type, 'Element', 'right type')
    ok(element.xml, 'XML mode active')
    element = dom.at('XMLTest').children()[0]
    is_ok(element.type, 'Element', 'right child')
    is_ok(element.parent.type, 'XMLTest', 'right parent')
    ...
    is_ok(dom.children()[0].type, 'a', 'right element')
    is_ok(dom.all_text, 'Test', 'right text')
    is_ok(dom.at('c').parent.type, 'b', 'right element')
    is_ok(dom.at('d').parent.type, 'b', 'right element')
    dom.at('b').prepend_content('<e>Mojo</e>')
    is_ok(dom.at('e').parent.type, 'b', 'right element')
    is_ok(dom.all_text, 'Mojo Test', 'right text')

    # Wrap elements
    dom = Pyjo.DOM.new('<a>Test</a>')
    is_ok(dom.wrap('<b></b>').node, 'root', 'right node')
    is_ok(dom, '<b><a>Test</a></b>', 'right result')
    is_ok(dom.at('b').strip().at('a').wrap('A').type, 'a', 'right element')
    is_ok(dom, '<a>Test</a>', 'right result')
    is_ok(dom.at('a').wrap('<b></b>').type, 'a', 'right element')
    is_ok(dom, '<b><a>Test</a></b>', 'right result')
    is_ok(dom.at('a').wrap('C<c><d>D</d><e>E</e></c>F').parent.type, 'd', 'right element')
    is_ok(dom, '<b>C<c><d>D<a>Test</a></d><e>E</e></c>F</b>', 'right result')

    # Wrap content
    dom = Pyjo.DOM.new('<a>Test</a>')
    is_ok(dom.at('a').wrap_content('A').type, 'a', 'right element')
    is_ok(dom, '<a>Test</a>', 'right result')
    is_ok(dom.wrap_content('<b></b>').node, 'root', 'right node')
    is_ok(dom, '<b><a>Test</a></b>', 'right result')
    is_ok(dom.at('b').strip().at('a').set(type='e:a').wrap_content('1<b c="d"></b>').type, 'e:a', 'right element')
    is_ok(dom, '<e:a>1<b c="d">Test</b></e:a>', 'right result')
    is_ok(dom.at('a').wrap_content('C<c><d>D</d><e>E</e></c>F').parent.node, 'root', 'right node')
