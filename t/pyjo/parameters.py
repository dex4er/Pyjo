...
if __name__ == '__main__':
    ...
    is_ok(params, 'foo=b%3Bar&baz=23', 'right format')
    is_ok(params2, 'x=1&y=2', 'right format')
    is_ok(params3, 'a=1&b=2&c=3&d=4', 'right format')
    is_ok(params, 'foo=b%3Bar&baz=23', 'right format')
    is_deeply_ok(params.params, [('foo', 'b;ar'), ('baz', '23')], 'right structure')

    # Append
    params.append(('a', 4), ('a', 5), ('b', 6), ('b', 7))
    is_ok(params, 'foo=b%3Bar&baz=23&a=4&a=5&b=6&b=7', 'right format')
    params.append(('c', 'f;oo'),)
    is_ok(params, 'foo=b%3Bar&baz=23&a=4&a=5&b=6&b=7&c=f%3Boo', 'right format')
    is_ok(params.remove('a'), 'foo=b%3Bar&baz=23&b=6&b=7&c=f%3Boo', 'right format')

    # Clone
    clone = params.clone()
    is_ok(params, clone, 'equal parameters')
    clone.append(('c', 9),)
    isnt_ok(params, clone, 'unequal parameters')

    # Merge
    params = Pyjo.Parameters.new('foo=b%3Bar&baz=23&a=4&a=5&b=6&b=7&c=f%3Boo')
    params.merge(params2)
    is_ok(params, 'foo=b%3Bar&baz=23&a=4&a=5&b=6&b=7&c=f%3Boo&x=1&y=2', 'right format')
    is_ok(params2, 'x=1&y=2', 'right format')
    is_ok(params.merge(('baz', None),), 'foo=b%3Bar&a=4&a=5&b=6&b=7&c=f%3Boo&x=1&y=2', 'right format')
    ...
    is_ok(params.remove('r'), 'q=1&w=2&e=3&e=4&t=7', 'right format')
    params.remove('e')
    is_ok(params, 'q=1&w=2&t=7', 'right format')
    ...
    is_ok(params, 'foo=&bar=bar', 'right format')
    params = Pyjo.Parameters.new(('bar', 'bar'), ('foo', ''))
    is_ok(params, 'bar=bar&foo=', 'right format')
    ...
    is_ok(params, 'foo=0', 'right format')
    ...
    is_ok(params, 'foo=bar&baz=23', 'right format')
    params = Pyjo.Parameters.new('foo=bar;baz=23')
    is_ok(params, 'foo=bar;baz=23', 'right format')
    ...
    params.param('foo ', 'a')
    is_ok(params, 'foo=%2B&foo+=a', 'right format')
    params.remove('foo ')
    is_deeply_ok(params.to_dict(), {'foo': '+'}, 'right structure')
    ...
    is_ok(params, 'a=works+too', 'right format')
    ...
    params = Pyjo.Parameters.new()
    is_ok(params.param('foo', ['ba;r', 'baz']), 'foo=ba%3Br&foo=baz', 'right format')
