...
if __name__ == '__main__':
    ...
    is_ok(url.query, '_monkey=biz%3B&_monkey=23', 'right query')
    ...
    is_ok(url, 'http://sri:foobar@example.com:8080?a=1&b=2#23', 'right format')
    url.query = {'c': 3}
    is_ok(url, 'http://sri:foobar@example.com:8080?c=3#23', 'right format')
    ...
    is_ok(url.path, '/0', 'no path')
    is_ok(url.query, '0', 'right query')
    is_ok(url.fragment, '0', 'right fragment')
    is_ok(url, 'https://example.com/0?0#0', 'right format')
    ...
    is_ok(url.path, 'image/png;base64,helloworld123', 'right path')
    is_ok(url.query, '', 'no query')
    is_ok(url.fragment, None, 'no fragment')
    is_ok(url, 'data:image/png;base64,helloworld123', 'right format')
    url = url.clone()
    is_ok(url.scheme, 'DATA', 'right scheme')
    is_ok(url.protocol, 'data', 'right protocol')
    is_ok(url.userinfo, None, 'no userinfo')
    ...
    is_ok(url.path, 'sri@example.com', 'right path')
    is_ok(url, 'mailto:sri@example.com', 'right format')
    url = Pyjo.URL.new().parse('foo:/test/123?foo=bar#baz')
    is_ok(url.scheme, 'foo', 'right scheme')
    is_ok(url.protocol, 'foo', 'right protocol')
    is_ok(url.path, '/test/123', 'right path')
    is_ok(url.query, 'foo=bar', 'right query')
    is_ok(url.fragment, 'baz', 'right fragment')
    is_ok(url, 'foo:/test/123?foo=bar#baz', 'right format')
    is_ok(url.set(scheme='Bar').to_str(), 'bar:/test/123?foo=bar#baz', 'right format')
    ...
    is_ok(url.query, 'foo=bar', 'right query')
    is_ok(url.fragment, 'baz', 'right fragment')
    is_ok(url, 'bar:/test/123?foo=bar#baz', 'right format')
    url = Pyjo.URL.new().parse('file:///foo/bar')
    is_ok(url.scheme, 'file', 'right scheme')
    is_ok(url.protocol, 'file', 'right protocol')
    is_ok(url.path, '/foo/bar', 'right path')
    is_ok(url, 'file:///foo/bar', 'right format')
    url = url.clone()
    is_ok(url.scheme, 'file', 'right scheme')
    is_ok(url.protocol, 'file', 'right protocol')
    is_ok(url.path, '/foo/bar', 'right path')
    ...
    is_ok(url.host, 'localhost', 'right host')
    is_ok(url.path, '/23/', 'right path')
    is_ok(url, '//localhost/23/', 'right relative version')
    is_ok(url.to_abs(Pyjo.URL.new('http://')), 'http://localhost/23/', 'right absolute version')
    is_ok(url.to_abs(Pyjo.URL.new('https://')), 'https://localhost/23/', 'right absolute version')
    is_ok(url.to_abs(Pyjo.URL.new('http://mojolicio.us')), 'http://localhost/23/', 'right absolute version')
    is_ok(url.to_abs(Pyjo.URL.new('http://mojolicio.us:8080')), 'http://localhost/23/', 'right absolute version')
    url = Pyjo.URL.new('///bar/23/')
    ok(not url.is_abs(), 'is not absolute')
    is_ok(url.host, '', 'no host')
    is_ok(url.path, '/bar/23/', 'right path')
    is_ok(url, '///bar/23/', 'right relative version')
    url = Pyjo.URL.new('////bar//23/')
    ok(not url.is_abs(), 'is not absolute')
    is_ok(url.host, '', 'no host')
    is_ok(url.path, '//bar//23/', 'right path')
    ...
    url.path = 'bar'
    is_ok(url, 'http://example.com/foo/bar?foo=bar#23', 'right path')
    url = Pyjo.URL.new('http://example.com?foo=bar#23')
    url.path = 'bar'
    is_ok(url, 'http://example.com/bar?foo=bar#23', 'right path')
    url = Pyjo.URL.new('http://example.com/foo?foo=bar#23')
    url.path = 'bar'
    is_ok(url, 'http://example.com/bar?foo=bar#23', 'right path')
    url = Pyjo.URL.new('http://example.com/foo/bar?foo=bar#23')
    url.path = 'yada/baz'
    is_ok(url, 'http://example.com/foo/yada/baz?foo=bar#23', 'right path')
    url = Pyjo.URL.new('http://example.com/foo/bar?foo=bar#23')
    url.path = '../baz'
    is_ok(url, 'http://example.com/foo/../baz?foo=bar#23', 'right path')
    url.path.canonicalize()
    is_ok(url, 'http://example.com/baz?foo=bar#23', 'right absolute path')

    # Absolute (base without trailing slash)
    url = Pyjo.URL.new('/foo?foo=bar#23')
    url.base.parse('http://example.com/bar')
    ok(not url.is_abs(), 'not absolute')
    is_ok(url.to_abs(), 'http://example.com/foo?foo=bar#23', 'right absolute version')
    url = Pyjo.URL.new('../cages/birds.gif')
    url.base.parse('http://www.aviary.com/products/intro.html')
    ok(not url.is_abs(), 'not absolute')
    is_ok(url.to_abs(), 'http://www.aviary.com/cages/birds.gif', 'right absolute version')
    url = Pyjo.URL.new('.././cages/./birds.gif')
    url.base.parse('http://www.aviary.com/./products/./intro.html')
    ok(not url.is_abs(), 'not absolute')
    is_ok(url.to_abs(), 'http://www.aviary.com/cages/birds.gif', 'right absolute version')
    ...
    ok(not url.is_abs(), 'not absolute')
    is_ok(url.to_abs(), 'http://example.com/bar/foo?foo=bar#23', 'right absolute version')
    is_ok(url.to_abs().base, 'http://example.com/bar/baz/', 'right base')
    ...
    is_ok(clone.path, '/test/index.html', 'right path')
    is_ok(clone.query, 'monkey=biz&foo=1', 'right query')
    is_ok(clone.fragment, '23', 'right fragment')
    is_ok(clone, 'ws://sri:foobar@example.com:8080/test/index.html?monkey=biz&foo=1#23', 'right format')
    clone.path = '/index.xml'
    is_ok(clone, 'ws://sri:foobar@example.com:8080/index.xml?monkey=biz&foo=1#23', 'right format')

    # Clone (with base)
    url = Pyjo.URL.new('/test/index.html')
    url.base.parse('http://127.0.0.1')
    is_ok(url, '/test/index.html', 'right format')
    clone = url.clone()
    is_ok(url, '/test/index.html', 'right format')
    ...
    is_ok(clone.path, '/test/index.html', 'right path')
    is_ok(clone.to_abs().to_str(), 'http://127.0.0.1/test/index.html', 'right absolute version')
    ...
    is_ok(clone.base.host, '127.0.0.1', 'right base host')
    is_ok(clone.path, 'test/index.html', 'right path')
    is_ok(clone.to_abs().to_str(), 'http://127.0.0.1/foo/test/index.html', 'right absolute version')
    ...
    is_ok(url.path, '/', 'right path')
    is_ok(url, 'wss://[::1]:3000/', 'right format')
    ...
    is_ok(url.path, '/foo', 'right path')
    is_ok(url.path_query, '/foo', 'right path and query')
    is_ok(url, 'http://xn--bcher-kva.ch:3000/foo', 'right format')
    ...
    is_ok(url.port, 3000, 'right port')
    is_ok(url.path, '/foo', 'right path')
    is_ok(url, 'http://xn--bcher-kva.xn--bcher-kva.ch:3000/foo', 'right format')
    ...
    is_ok(url.port, 3000, 'right port')
    is_ok(url.path, '/foo', 'right path')
    is_ok(url, 'http://xn--bcher-kva.xn--bcher-kva.xn--bcher-kva.ch:3000/foo', 'right format')
    ...
    is_ok(url.port, 3000, 'right port')
    is_ok(url, 'https://foo:b%C3%A4r@xn--krih-moa.com:3000', 'right format')
    ...
    is_ok(url.path, '/', 'right path')
    is_ok(url, 'http://xn--n3h.net/', 'right format')

    # IRI/IDNA
    url = Pyjo.URL.new(u'http://☃.net/♥/?q=♥☃')
    is_ok(url.path.parts[0], u'♥', 'right path part')
    is_ok(url.path, '/%E2%99%A5/', 'right path')
    is_ok(url.query, 'q=%E2%99%A5%E2%98%83', 'right query')
    ...
    is_ok(url.host, u'☃.Net', 'right host')
    is_ok(url.ihost, 'xn--n3h.net', 'right internationalized host')
    is_ok(url.path, '/%E2%99%A5/%E2%99%A5/', 'right path')
    is_deeply_ok(url.path.parts, [u'♥', u'♥'], 'right structure')
    is_ok(url.query.param(u'♥'), u'☃', 'right query value')
    is_ok(url, 'http://xn--n3h.net/%E2%99%A5/%E2%99%A5/?%E2%99%A5=%E2%98%83', 'right format')
    ...
    is_ok(url.to_abs(), 'http://foo.com/', 'right absolute version')

    # Empty path elements
    url = Pyjo.URL.new('http://example.com/foo//bar/23/')
    ok(url.is_abs(), 'is absolute')
    is_ok(url.path, '/foo//bar/23/', 'right path')
    is_ok(url, 'http://example.com/foo//bar/23/', 'right format')
    url = Pyjo.URL.new('http://example.com//foo//bar/23/')
    ok(url.is_abs(), 'is absolute')
    is_ok(url.path, '//foo//bar/23/', 'right path')
    is_ok(url, 'http://example.com//foo//bar/23/', 'right format')
    ...
    is_ok(url.base, '', 'no base')
    is_ok(url.scheme, 'http', 'right scheme')
    is_ok(url.userinfo, None, 'no userinfo')
    is_ok(url.host, 'foo.bar', 'right host')
    is_ok(url.port, None, 'no port')
    is_ok(url.path, '/baz', 'right path')
    is_ok(url.query, 'yada', 'right query')
    is_ok(url.fragment, None, 'no fragment')
    is_ok(url, 'http://foo.bar/baz?yada', 'right absolute URL')
    url = Pyjo.URL.new('zzz?Zzz').set(base=url).to_abs()
    is_ok(url.base, 'http://foo.bar/baz?yada', 'right base')
    is_ok(url.scheme, 'http', 'right scheme')
    is_ok(url.userinfo, None, 'no userinfo')
    is_ok(url.host, 'foo.bar', 'right host')
    is_ok(url.port, None, 'no port')
    is_ok(url.path, '/zzz', 'right path')
    is_ok(url.query, 'Zzz', 'right query')
    is_ok(url.fragment, None, 'no fragment')
    is_ok(url, 'http://foo.bar/zzz?Zzz', 'right absolute URL')
    ...
    is_ok(url.host, 'foo.bar', 'right host')
    is_ok(url.port, None, 'no port')
    is_ok(url.path, '/baz/index.html', 'right path')
    is_ok(url.query, 'yada', 'right query')
    is_ok(url.fragment, None, 'no fragment')
    is_ok(url, 'http://foo.bar/baz/index.html?yada', 'right absolute URL')
    url = Pyjo.URL.new('zzz?Zzz').set(base=url).to_abs()
    is_ok(url.base, 'http://foo.bar/baz/index.html?yada', 'right base')
    is_ok(url.scheme, 'http', 'right scheme')
    is_ok(url.userinfo, None, 'no userinfo')
    is_ok(url.host, 'foo.bar', 'right host')
    is_ok(url.port, None, 'no port')
    is_ok(url.path, '/baz/zzz', 'right path')
    is_ok(url.query, 'Zzz', 'right query')
    is_ok(url.fragment, None, 'no fragment')
    is_ok(url, 'http://foo.bar/baz/zzz?Zzz', 'right absolute URL')
    ...
    is_ok(url.query, '', 'no query')
    is_ok(url.fragment, None, 'no fragment')
    is_ok(url, 'http://foo.bar/zzz', 'right absolute URL')
    ...
    is_ok(url, 'http://foo.bar/baz/index.html?yada#test1', 'right absolute URL')
    url = Pyjo.URL.new('/zzz#test2').set(base=url).to_abs()
    is_ok(url.base, 'http://foo.bar/baz/index.html?yada#test1', 'right base')
    is_ok(url.scheme, 'http', 'right scheme')
    is_ok(url.userinfo, None, 'no userinfo')
    is_ok(url.host, 'foo.bar', 'right host')
    is_ok(url.port, None, 'no port')
    ...
    is_ok(url, 'http://foo.bar/zzz#test2', 'right absolute URL')
    ...
    is_ok(url.query, '', 'no query')
    is_ok(url.fragment, 'test2', 'right fragment')
    is_ok(url, 'http://foo.bar/baz/zzz#test2', 'right absolute URL')
    ...
    is_ok(url.query, '', 'no query')
    is_ok(url.fragment, None, 'no fragment')
    is_ok(url, 'http://foo.bar/baz/zzz', 'right absolute URL')
    ...
    is_ok(url.host, 'example.com', 'right host')
    is_ok(url.port, None, 'no port')
    is_ok(url.path, '/mojo%2Fg%2B%2B-4%2E2_4%2E2%2E3-2ubuntu7_i386%2Edeb', 'right path')
    is_ok(url.query, '', 'no query')
    is_ok(url.fragment, '', 'right fragment')
    is_ok(url, 'http://example.com/mojo%2Fg%2B%2B-4%2E2_4%2E2%2E3-2ubuntu7_i386%2Edeb#', 'right format')
    url.path.canonicalize()
    is_ok(url, 'http://example.com/mojo/g++-4.2_4.2.3-2ubuntu7_i386.deb#', 'right format')

    # "%" in path
    url = Pyjo.URL.new('http://mojolicio.us/100%_fun')
    is_ok(url.path.parts[0], '100%_fun', 'right part')
    is_ok(url.path, '/100%25_fun', 'right path')
    is_ok(url, 'http://mojolicio.us/100%25_fun', 'right format')
    url = Pyjo.URL.new('http://mojolicio.us/100%fun')
    is_ok(url.path.parts[0], '100%fun', 'right part')
    is_ok(url.path, '/100%25fun', 'right path')
    is_ok(url, 'http://mojolicio.us/100%25fun', 'right format')
    ...
    is_ok(url.query, '%E5=%E4', 'right query')
    is_ok(url.query.param(b"\xe5"), b"\xe4", 'right value')
    is_ok(url, 'http://foo.bar/%E4/?%E5=%E4', 'right format')

    # Resolve RFC 1808 examples
    base = Pyjo.URL.new('http://a/b/c/d?q#f')
    url = Pyjo.URL.new('g')
    is_ok(url.to_abs(base), 'http://a/b/c/g', 'right absolute version')
    url = Pyjo.URL.new('./g')
    is_ok(url.to_abs(base), 'http://a/b/c/g', 'right absolute version')
    url = Pyjo.URL.new('g/')
    is_ok(url.to_abs(base), 'http://a/b/c/g/', 'right absolute version')
    url = Pyjo.URL.new('//g')
    is_ok(url.to_abs(base), 'http://g', 'right absolute version')
    url = Pyjo.URL.new('?y')
    is_ok(url.to_abs(base), 'http://a/b/c/d?y', 'right absolute version')
    url = Pyjo.URL.new('g?y')
    is_ok(url.to_abs(base), 'http://a/b/c/g?y', 'right absolute version')
    url = Pyjo.URL.new('g?y/./x')
    is_ok(url.to_abs(base), 'http://a/b/c/g?y/./x', 'right absolute version')
    url = Pyjo.URL.new('#s')
    is_ok(url.to_abs(base), 'http://a/b/c/d?q#s', 'right absolute version')
    url = Pyjo.URL.new('g#s')
    is_ok(url.to_abs(base), 'http://a/b/c/g#s', 'right absolute version')
    url = Pyjo.URL.new('g#s/./x')
    is_ok(url.to_abs(base), 'http://a/b/c/g#s/./x', 'right absolute version')
    url = Pyjo.URL.new('g?y#s')
    is_ok(url.to_abs(base), 'http://a/b/c/g?y#s', 'right absolute version')
    url = Pyjo.URL.new('.')
    is_ok(url.to_abs(base), 'http://a/b/c', 'right absolute version')
    url = Pyjo.URL.new('./')
    is_ok(url.to_abs(base), 'http://a/b/c/', 'right absolute version')
    url = Pyjo.URL.new('..')
    is_ok(url.to_abs(base), 'http://a/b', 'right absolute version')
    url = Pyjo.URL.new('../')
    is_ok(url.to_abs(base), 'http://a/b/', 'right absolute version')
    url = Pyjo.URL.new('../g')
    is_ok(url.to_abs(base), 'http://a/b/g', 'right absolute version')
    url = Pyjo.URL.new('../..')
    is_ok(url.to_abs(base), 'http://a/', 'right absolute version')
    url = Pyjo.URL.new('../../')
    is_ok(url.to_abs(base), 'http://a/', 'right absolute version')
    url = Pyjo.URL.new('../../g')
    is_ok(url.to_abs(base), 'http://a/g', 'right absolute version')
